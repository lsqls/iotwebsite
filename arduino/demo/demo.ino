...
SMSGSM sms;
InetGSM inet;
int smokepin = A0;//测量A0引脚的电压，即烟雾传感器的数值
int numdata;
...
void loop()
{
  ...
  if (value > 1000)  sendSMS("13051680866", "fire");//假如测量值大于1000，视为有火灾发生，向用户报警
  ggps();//获取gps数据
  valuestr = dtostrf(value, 3, 2, buffer);//将浮点数转化为字符串
  if (Save_Data.isUsefull)
    sprintf(datastr, "/smoke/upload/%s-%s-%s", valuestr, Save_Data.longitude, Save_Data.latitude);//能接收gps信号，构建含gps数据和烟雾传感器所测数据的字符串
  else
    sprintf(datastr, "/smoke/upload/%s", valuestr);//能接收gps信号，构建只含烟雾传感器数据的字符串
